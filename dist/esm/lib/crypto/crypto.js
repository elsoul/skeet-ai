...
export const encrypt = (data, iv, password, salt) => {
    try {
        const key = scryptSync(password, salt, 32);
        const cipher = createCipheriv(algorithm, key, Buffer.from(iv, outputEncoding));
...
export const decrypt = (data, iv, password, salt) => {
    try {
        const key = scryptSync(password, salt, 32);
        const decipher = createDecipheriv(algorithm, key, Buffer.from(iv, outputEncoding));
